"""Command-line script for publishing FCHL models to DLHub"""
...
parser = ArgumentParser(description='''Post a FCHL-based delta-learning model to DLHub.
...
model_file = os.path.join(args.directory, 'model.pkl.gz')
metadata_file = os.path.join(args.directory, 'about.yml')
if not (os.path.isfile(model_file) and os.path.isfile(metadata_file)):
    raise ValueError('Directory must contain "model.pkl.gz" and "about.yml" with model and metadata"')
...
if os.path.lexists('model.pkl.gz'):
    os.unlink('model.pkl.gz')
os.symlink(model_file, 'model.pkl.gz')
with open('fidelity.json', 'w') as fp:
    json.dump(metadata['input_fidelity'], fp)

model.add_file('dlhub_app.py')
model.add_file('model.pkl.gz')
model.add_file('fidelity.json')
...
#   Model name
model.set_name(f'g4mp2_delta_fchl_{metadata["input_fidelity"].lower()}')
model.set_title(f'FCHL Model to Predict G4MP2 Atomization Energy from {metadata["input_fidelity"]} Total Energy')
